...
{
	return ( ( spawnpoint.GetOrigin().z > GetLethalFogTop() ) && ( (GetLethalFogTop() + 300 ) > spawnpoint.GetOrigin().z ) ) // OG spawnpoint.GetOrigin().z > GetLethalFogTop()
}
...
void function InitLavaFogController( entity fogController )
{
	...
	fogController.kv.foghalfdisttop = "60000" // OG 60000
	fogController.kv.foghalfdistbottom = "200" // OG 200
	fogController.kv.fogdistoffset = "0" // OG 0
	fogController.kv.fogdensity = ".85" // OG .85

	fogController.kv.forceontosky = true // fog renders over skybox/out of bounds map assets / OG true
	//fogController.kv.foghalfdisttop = "10000"
}

void function CreateCustomSpawns() // Still need to fix spawning behind walls on at least mp_wargames and mp_glitch | also, would like to add enemy team proximity or just insert this whole section as a "if fog" in the spawn.nut
{
...
void function CreateCustomSpawns_Threaded()
{
	...
	float raycastTop = GetLethalFogTop() + 545 // OG 2500, may not change anything but didn't break anything either
